    ...
    function __construct ()
    {
        ...
        if (in_array($_PE[1], array('rest', 'rest.php'))) {
            ...
            if (!isset($this->aResourcesSupported[$this->sResource])) {
                // MODIFIED: api.lovd.nl allows listing the available resources. This feature is not available in LOVD3.
                if (!$this->sResource) {
                    $this->aResponse['errors'][] = 'Failed to pass required resource name. Please choose from: ' .
                        implode(', ', array_keys($this->aResourcesSupported)) . '.';
                } else {
